...
SOAP_SOURCE_STAMP("@(#) soapServer.cpp ver 2.8.124 2023-02-04 11:42:21 GMT")
...
SOAP_FMAC5 int SOAP_FMAC6 soap_serve___tds__SetHashingAlgorithm(struct soap *soap)
{	struct __tds__SetHashingAlgorithm soap_tmp___tds__SetHashingAlgorithm;
	...
	soap_default___tds__SetHashingAlgorithm(soap, &soap_tmp___tds__SetHashingAlgorithm);
	if (!soap_get___tds__SetHashingAlgorithm(soap, &soap_tmp___tds__SetHashingAlgorithm, "-tds:SetHashingAlgorithm", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = __tds__SetHashingAlgorithm(soap, soap_tmp___tds__SetHashingAlgorithm.tds__SetHashingAlgorithm, tds__SetHashingAlgorithmResponse);
	if (soap->error)
		return soap->error;
	soap->encodingStyle = NULL; /* use SOAP literal style */
	soap_serializeheader(soap);
	tds__SetHashingAlgorithmResponse.soap_serialize(soap);
	if (soap_begin_count(soap))
		return soap->error;
	if ((soap->mode & SOAP_IO_LENGTH))
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || tds__SetHashingAlgorithmResponse.soap_put(soap, "tds:SetHashingAlgorithmResponse", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
